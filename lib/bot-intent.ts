...
/**
 * ตาข่ายรองรับเจตนา "อยากลอง/อยากสมัคร" แบบกำหนดตายตัว
 *
 * เดิมโค้ดทดลองจะออกก็ต่อเมื่อโมเดลเติม marker `[INTENT:trial]` มาเอง ซึ่งใน
 * ทางปฏิบัติแทบไม่เกิด — วัดจากของจริง: lead จากแชท ~41 คนในสองเดือนครึ่ง ได้โค้ด
 * ไปแค่ 1 คน (2.4%) ที่เหลือค้างที่ stage `new` ตลอดไป เพราะ cron ติดตามผลจับ
 * เฉพาะ `code_issued` จึงไม่มีอะไรมาเก็บกวาดต่อ
 *
 * ตัวนี้ไม่ได้แทนที่ marker ของโมเดล แต่เป็นชั้นล่างที่จับคำที่บอกเจตนาเชิงพาณิชย์
 * ชัดเจน (ขอโค้ด / ถามราคา / อยากสมัคร) การยิงซ้ำปลอดภัยอยู่แล้วเพราะ
 * handleBotIntent จะส่งโค้ดเดิมคืนถ้ายังไม่หมดอายุ, ข้าม lead ที่แปลงแล้ว และ
 * จำกัดที่ MAX_CODES_PER_LEAD
 *
 * ตั้งใจ **ไม่** จับคำว่า "ลอง" เดี่ยว ๆ เพราะชนกับคำถามทางคลินิกบ่อย
 * ("ลองให้ยาอะไรก่อน", "ลองตรวจอะไรดี")
 */
const TRIAL_INTENT_PATTERNS: RegExp[] = [
  /ทดลอง/,                          // ทดลองใช้, ขอทดลอง
  /โค้ด|โคด|คูปอง|coupon|code/i,     // ขอโค้ด, มีโค้ดไหม
  /สมัคร/,                           // สมัครยังไง, อยากสมัคร, สมัครฟรี
  /ราคา|กี่บาท|เท่าไหร่|เท่าไร/,      // ถามราคา = เจตนาเชิงพาณิชย์
  /แพ็?กเกจ|แพคเกจ|ค่าสมาชิก|รายเดือน|รายปี/,
  /(อยาก|ขอ|จะ|ช่วย)\s*ลอง/,        // "ลอง" ต้องมาคู่กับคำบอกเจตนา
  /ลองใช้|ลองเล่น/,
  /สนใจ/,
  /เริ่ม(ต้น)?(ยัง|ไง|ย)/,           // เริ่มยังไง, เริ่มต้นยังไง
];

/**
 * true เมื่อข้อความของผู้ใช้บอกเจตนาอยากทดลอง/สมัครอย่างชัดเจนพอที่จะเสนอโค้ดให้
 */
export function detectTrialIntent(message: string): boolean {
  const text = message.trim();
  if (!text) return false;
  // ข้อความที่เป็นอีเมลล้วนคือการให้ข้อมูล ไม่ใช่การขอโค้ด — ปล่อยให้
  // handleEmailCapture จัดการ ไม่งั้นจะตอบซ้อนกันสองเรื่องในทีเดียว
