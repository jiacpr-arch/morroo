...
import { OPERATIONS, getBuiltinOperation } from "./operations";
...
describe("pointerDown — เครื่องมือ/ตำแหน่ง", () => {
  ...
  it("wrongToolWhy ถูกส่งเป็น note เมื่อกำหนดไว้", () => {
    const op = getBuiltinOperation("suture-laceration-01")!;
    const st = freshState();
...
describe("OPERATIONS — ความถูกต้องของข้อมูลด่าน", () => {
  it("มี 3 ด่าน slug ไม่ซ้ำ", () => {
    expect(OPERATIONS.length).toBe(3);
    expect(new Set(OPERATIONS.map((o) => o.slug)).size).toBe(3);
  });

  it.each(OPERATIONS.map((op) => [op.slug, op] as const))("%s ผ่าน isValidOperation", (_slug, op) => {
    expect(isValidOperation(op)).toBe(true);
  });

  it.each(OPERATIONS.map((op) => [op.slug, op] as const))(
