// BLS decision-game — 8 ด่าน + Final Exam (port จาก acls-emr, ILCOR 2025)
//
// แต่ละด่านคือเคสต่อเนื่องหนึ่งเคส เดินตามผู้ป่วยรายเดียวทีละจังหวะ
// แต่ละ step มีคำถามจับเวลา เลือกครั้งแรกล็อกคำตอบ (ไม่มี retry) แล้วเฉลย
// เนื้อหา rate/depth/ratio และ "trap" อิงบทเรียน BLS-HCP ที่ตรวจแล้ว
// ตัวเลือกถูกเขียนไว้ตัวแรกเสมอ — getStageById สลับลำดับตอนโหลด
//
// ids ใช้ prefix bls- เพื่อไม่ชนกับ scenario ปกติของ firstaid
// (fa_simulation_runs.scenario_id เป็น free text; id เหล่านี้ไม่อยู่ใน
// scenariosById จึงไม่กระทบเงื่อนไข certificate/practice)

export interface BlsOption {
  label: string;
  correct?: boolean;
  feedback: string;
  trap?: boolean;
...
export interface BlsStep {
  situation: string;
  question: string;
  timeLimitSec?: number;
  /** หลังตอบถูก: ฝึกจับจังหวะกดหน้าอก 30 วิ (metronome 110/นาที) ก่อนไปต่อ */
  cprDrill?: boolean;
  /** ใส่โดย buildFinalExam เพื่อบอกว่าข้อนี้มาจากด่านไหน */
  sourceStage?: string;
}

export interface BlsStageStep extends BlsStep {
  options: BlsOption[];
}

export interface BlsStage {
  id: string;
...
export const BLS_STAGES: BlsStage[] = [
  ...
  {
    id: 'bls-stage-6-inhospital-defib',
    chapterId: 'bls-5',
    stageNumber: 6,
...
// ---- Final exam: ทบทวนต่อเนื่องทุกเคส --------------------------------------
// ไล่ตามลำดับเคสแล้วหยิบ step เปิดเรื่องของแต่ละด่าน — ข้อสอบยังอ่านเป็น
// สถานการณ์ไหลต่อกันทีละเคส ไม่ใช่คำถามสุ่มปน

export const BLS_FINAL_ID = 'bls-final';
...
export function buildFinalExam(): BlsStage {
  const picked = BLS_STAGES.flatMap((stage) =>
    stage.steps.slice(0, FINAL_EXAM_PER_STAGE).map((s) => ({
      ...s,
      cprDrill: false, // drill เป็นของด่านย่อย ไม่ใช่ข้อสอบรวม
  ...
  return {
    id: BLS_FINAL_ID,
    chapterId: null,
    stageNumber: BLS_STAGES.length + 1,
...
// สลับลำดับตัวเลือกตอนโหลดด่าน — ข้อมูลเขียนข้อถูกไว้ตัวแรกเสมอ
// ผู้เล่นต้องเดาตำแหน่งไม่ได้ (runner memoize ต่อการโหลด ลำดับนิ่งระหว่างเล่น)
function shuffleOptions(step: BlsStageStep): BlsStageStep {
  return { ...step, options: shuffle(step.options) };
}

export function getStageById(id: string): BlsStage | null {
  const stage = id === BLS_FINAL_ID
    ? buildFinalExam()
    : BLS_STAGES.find((s) => s.id === id) ?? null;
  if (!stage) return null;
  return { ...stage, steps: stage.steps.map(shuffleOptions) };
}

// ---- Unlock logic (จาก passedScenarioIds ใน progressStore) ------------------
// ปลดล็อกตามลำดับ: ด่าน 1 เปิดเสมอ, ด่าน n ต้องผ่านด่าน n-1 ก่อน
// Final Exam ต้องผ่านครบทั้ง 8 ด่าน

export function isBlsStageUnlocked(stageNumber: number, passedIds: ReadonlySet<string>): boolean {
  if (stageNumber <= 1) return true;
  const prev = BLS_STAGES.find((s) => s.stageNumber === stageNumber - 1);
  return !!prev && passedIds.has(prev.id);
}

export function isFinalUnlocked(passedIds: ReadonlySet<string>): boolean {
  return BLS_STAGES.every((s) => passedIds.has(s.id));
}

export function countPassedStages(passedIds: ReadonlySet<string>): number {
  return BLS_STAGES.filter((s) => passedIds.has(s.id)).length;
}
