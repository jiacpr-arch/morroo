...
const certConfig = getCertConfig("bls");
const courseMeta = getCourseMeta("bls");
...
export default function BlsCertificationPage() {
  ...
  useEffect(() => {
    if (certData.certId && !lineUnlocked) {
      track("cert_line_gate_view", { source: "cert_gate", course: "bls" });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  // Online certification — two knowledge gates (pre-course, post-test). BLS
  // has no pre-test (unlike ACLS) and no separate EKG skill test.
  const requirements = [
    {
  ...
  const generateCertificate = async () => {
    ...
    // BLS has no pre-test — preTestScore is always null in the cert record
    // and cert-notify payload.
    const data: CertData = {
      studentName: name,
      studentPhone: tel,
      studentEmail: mail,
  ...
  const unlockDownload = (via: "line" | "skip") => {
    ...
    if (via === "skip") {
      track("cert_line_skip", { source: "cert_gate", course: "bls" });
    }
  };

  const handleAddLine = () => {
    track("cert_line_add", { channel: "line", source: "cert_gate", course: "bls" });
    unlockDownload("line");
  };

  const downloadPDF = async () => {
    track("cert_download", { source: "cert_card", course: "bls" });
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className={cn(dashCard, "!p-3 text-center")}>
            <div
              className={cn(
                "text-lg font-bold",
                preCourseDone ? "text-emerald-600 dark:text-emerald-400" : "text-amber-600 dark:text-amber-400",
              )}
            >
              {preCourseStatus.filter((s) => s.passed).length}/{preCourseStatus.length}
            </div>
            <div className="text-[11px] text-muted-foreground">บทเรียนผ่าน</div>
          </div>
          <div className={cn(dashCard, "!p-3 text-center")}>
            <div
              className={cn(
