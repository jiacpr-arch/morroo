...
| Ad **`★ CG Feed A — โทนมืด`** | `52588606335197` | PAUSED |
| Ad **`★ CG Feed B — โทนสดใส`** | `52588606394397` | PAUSED |
...
**เปิดแค่ 4 ตัวที่ขึ้นต้นด้วย ★** — ทุกตัวยิงเข้า Direct (เคส torsion) เหมือนกันหมด ต่างกันแค่**สไตล์ภาพ** (มืด vs สดใส) และ**สัดส่วน** (feed 4:5 vs story 9:16) แยกกันด้วย `utm_content` = `feed_a_dark` / `feed_b_bright` / `story_a_dark` / `story_b_bright` → เทียบได้ตรงๆ ว่าสไตล์ไหนได้ `casegame_first_decision` ต่อบาทถูกกว่า

**ยังไม่เปิด Hub** — ตอนนี้ Direct ส่งคนไปที่เคสเขียนมือซึ่งเป็นเคสคุณภาพดีที่สุดที่มี และ copy ตรงกับเคสนั้นพอดี ทางขยายคือทำ Direct เพิ่มยิงเข้าคนละเคส ไม่ใช่ไปเปิด Hub — จะได้รู้ด้วยว่าเคสไหนขายดี

⚠️ **ad ทั้ง 4 อยู่ ad set เดียวกันที่งบ ฿120/วัน** ซึ่งค่อนข้างบางสำหรับการเรียนรู้ 4 creative พร้อมกัน ถ้าอยากได้ผลเร็วให้เปิดทีละคู่ (feed ก่อน แล้วค่อย story) หรือเพิ่มงบ
