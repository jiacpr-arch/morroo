...
>
> **อัปเดต (20 ก.ย.) — reader เปลี่ยนเป็น "mini class" card deck:** ผู้เรียนเห็นทีละ Part
> (เปลี่ยนชื่อเป็น "ส่วน") บนหน้าเดียว ไม่ใช่อ่านยาวรวดเดียวเหมือนเดิม (`LessonReader.tsx`,
> `layout="deck"` สำหรับนักเรียน / `layout="stacked"` สำหรับแอดมินรีวิว) และทุกส่วนต้องมีคำถามท้ายส่วน
> รวมส่วนสุดท้ายด้วย (trailing gate — ดู `lib/school/lesson-parts.ts`, `lib/school/lesson-deck.ts`)
> ส่วนจะสั้นลงเหลือ 60–150 คำ/ส่วน (6–12 ส่วนต่อบท แทน 3–4 Part เดิม) — บทเก่าจะถูกแบ่งใหม่ทีละวิชา
> ด้วย `npm run resplit:parts` (ดู `docs/school-lesson-template.md`) **"1 Part = 1 รูป" ในแผนนี้จึงหมายถึง
> 1 ช่องรูปต่อ 1 ส่วนสั้น** (ไม่ใช่ 1 รูปต่อ Part ยาวแบบเดิม) — ตำแหน่ง/จำนวนรูปต่อบท (ข้อ 1, 2) ยังใช้หลักเดิม
> แค่นับต่อ "ส่วน" แทน "Part" ยาว ๆ; รันสคริปต์ `gen:figures` **หลัง** resplit วิชานั้นแล้วจะได้ diagram ละเอียดขึ้น
> (1 diagram ต่อ 1 ส่วนสั้น แทน 1 ต่อ Part ยาว) แต่รันก่อนก็ยังทำงานได้ (parser รองรับทั้งสองแบบ)
